...
struct myUnit {
    ...
    // +
    myUnit<num, denum, m_exp, kg_exp, s_exp> 
    operator+(myUnit<num, denum, m_exp, kg_exp, s_exp> rhs) {
        return myUnit<num, denum, m_exp, kg_exp, s_exp>{ repr + rhs.repr };
    }
    ...
    // *
    template<uint32_t num2, uint32_t denum2, int m2_exp, int s2_exp, int kg2_exp>
    myUnit<num*num2, denum*denum2, m_exp + m2_exp, kg_exp + s2_exp, s_exp + kg2_exp> 
    operator*(
        myUnit<num2, denum2, m2_exp, s2_exp, kg2_exp> rhs
    ) {
        return myUnit<num*num2, denum*denum2, m_exp + m2_exp, kg_exp + s2_exp, s_exp + kg2_exp>{ repr * rhs.repr };
    }

    // /
    template<uint32_t num2, uint32_t denum2, int m2_exp, int s2_exp, int kg2_exp>
    myUnit<num*denum2, denum*num2, m_exp - m2_exp, kg_exp - s2_exp, s_exp - kg2_exp> 
    operator/(
        myUnit<num2, denum2, m2_exp, s2_exp, kg2_exp> rhs
    ) {
        return myUnit<num*denum2, denum*num2, m_exp - m2_exp, kg_exp - s2_exp, s_exp - kg2_exp>{ repr / rhs.repr };
    }

    inline constexpr bool operator==(const myUnit<num, denum, m_exp, kg_exp, s_exp> rhs) const {
...
namespace sit {
    ...
    typedef myUnit< 1000,    1, 1, 0, 0> KiloMetre_t;
    typedef myUnit<    1,   10, 1, 0, 0> DeciMetre_t;
    typedef myUnit<    1,  100, 1, 0, 0> CentiMetre_t;

    typedef myUnit< 1000, 3600, 1, 0,-1> KiloMetrePerHour_t;

    typedef myUnit<   60,    1, 0, 0, 1> Minute_t;
    typedef myUnit< 3600,    1, 0, 0, 1> Hour_t;

    typedef myUnit< 1, 1, 1, 0, 0> Metre_t;
    typedef myUnit< 1, 1, 0, 0, 1> Second_t;
    typedef myUnit< 1, 1, 0, 1, 0> Kilogram_t;
                        
    typedef myUnit< 1, 1, 2, 0, 0> Metre2_t;
    typedef myUnit< 1, 1, 0, 0, 2> Second2_t;
    typedef myUnit< 1, 1, 0, 2, 0> Kilogram2_t;
                        
    typedef myUnit< 1, 1, 3, 0, 0> Metre3_t;
    typedef myUnit< 1, 1, 0, 0, 3> Second3_t;
    typedef myUnit< 1, 1, 0, 3, 0> Kilogram3_t;
